...
import SortByComponent, {
  SortField,
  SortOrder,
} from "@/components/practice/SortByComponent";
...
const SAMPLE_QUESTIONS: Question[] = [
  ...
  },
  {
    id: "q2",
    title:
      "A convoy moving south at the speed of 60 km/h collides with a missile moving north-west at 80 km/h. Calculate the time of collision.",
    difficulty: "Medium",
    tags: ["calculus", "integration"],
    paper: "JEE Advanced 2023",
  },
  {
    id: "q3",
  ...
  {
    id: "q4",
    title: "Multiple choice: identify the verb",
    difficulty: "Easy",
    tags: ["grammar", "mcq"],
    paper: "BITSAT 2024",
  },
...
export default function Home() {
  ...
  const filtered = useMemo(() => {
    ...
    if (query.trim()) {
      ...
      list = list.filter(
        (it) =>
          it.title.toLowerCase().includes(q) || it.tags.join(" ").includes(q),
      );
    }
  ...
  const cntnt = (
    <>
      <header className="mb-4 flex items-center justify-between">
        <div>
          ...
          <p className="mt-1 text-sm text-muted-foreground">
            Pick a filter or search to find questions
          </p>
        </div>
      ...
      <div className="space-y-3">
        {filtered.length === 0 ? (
          <div className="rounded-md border border-border p-4 text-muted-foreground">
            No questions match your search.
          </div>
        ) : (
          filtered.slice(0, displayCount).map((q) => {
            const attempts =
              q.id === "q1" ? 12 : q.id === "q2" ? 34 : q.id === "q3" ? 7 : 3;
            const accuracy =
              q.id === "q1"
                ? 0.82
                : q.id === "q2"
