...
export default function QuestionPage({ params }: PageProps) {
  ...
  const filteredQuestions = useMemo(() => {
    return SAMPLE_QUESTIONS.filter((q: Question) => {
      if (questionFilter === "attempted")
        return q.attempts && q.attempts.total > 0;
  ...
  const prevQuestion =
    questionIdx > 0 ? SAMPLE_QUESTIONS[questionIdx - 1] : null;
  const nextQuestion =
    questionIdx < SAMPLE_QUESTIONS.length - 1
      ? SAMPLE_QUESTIONS[questionIdx + 1]
      : null;
        ...
        <div className="space-y-3">
          ...
          <div className="flex items-center gap-6 text-sm">
            ...
            <div className="flex items-center gap-2">
              ...
              <span className="font-medium">
                {(
                  ((question.attempts?.correct || 0) /
                    (question.attempts?.total || 1)) *
                  100
                ).toFixed(1)}
                %
              </span>
                    ...
                    <div className="space-y-3">
                      {question.options.map((option, idx) => (
                        <button
                          ...
                          onClick={() =>
                            !isSubmitted &&
                            setAnswer({ ...answer, single: option })
                          }
                      ...
                      {isSubmitted && !isCorrect && (
                        <Button
                          ...
                          onClick={() => {
                            if (
                              window.confirm(
                                "Are you sure you want to see the solution? This will be counted as an unsuccessful attempt.",
                              )
                            ) {
        ...
        >
          <div className="border-b p-4">
            ...
            <div className="flex gap-2 overflow-x-auto pb-2">
              ...
              <Button
                variant={
                  questionFilter === "attempted" ? "secondary" : "outline"
                }
                size="sm"
              ...
              >
                Attempted (
                {
                  SAMPLE_QUESTIONS.filter(
                    (q) => q.attempts && q.attempts.total > 0,
                  ).length
                }
                )
              </Button>
              <Button
                variant={
                  questionFilter === "unattempted" ? "secondary" : "outline"
                }
                size="sm"
